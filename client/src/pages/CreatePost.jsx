...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        setForm({ ...form, photo: `data:image/jpeg;base,${data.photo}` });
      } catch (err) {
        console.error(err);
        alert(err);
      } finally {
  ...
  return (
    <section className="max-w-7xl mx-auto">
      <Header header="Create" text="Create imaginative and visually stunning images through DALL-E AI and share them with the community" />
      ...
      <form className="mt-16 max-w-3xl" onSubmit={handleSubmit}>
        <div className="flex flex-col gap-5">
          ...
          <div className="relative bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 w-64 p-3 h-64 flex justify-center items-center">
            ...
            {generatingImg && <div className="absolute inset-0 flex justify-center items-center bg-[rgba(0, 0, 0, 0.5)] rounded-lg"></div>}
          </div>
        </div>
